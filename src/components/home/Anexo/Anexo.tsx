...
export const Anexo = () => {
  return (
    <section id="otros-servicios" className="py-16 bg-surface border-t border-white/5">
      <div className="max-w-4xl mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {serviciosAnexo.map((s, i) => (
            ...
            >
              <div className="text-gray-400 flex-shrink-0 mt-0.5">{s.icon}</div>
              <div>
                <h3 className="text-gray-400 mb-1 tracking-wide uppercase">{s.titulo}</h3>
                <p className={sectionDescClass}>{s.descripcion}</p>
              </div>
            </div>
          ))}
        </div>
